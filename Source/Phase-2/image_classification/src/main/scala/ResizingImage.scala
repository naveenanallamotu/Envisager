...
object ResizingImage {
  ...
  def main(args: Array[String]) {
    val IMAGE_CATEGORIES = List("Computers", "crossing", "Cycles", "electronics", "female", "male", "Road", "Rocks", "Sidewalk", "Stairs", "vehicles", "Water")
    val PATH = "data/train/"
    val PATH2 = "data/train12/"
    IMAGE_CATEGORIES.foreach(f => {
      ...
      listOffiles.foreach(fi => {
        ...
        imwrite(file2.getPath + "/" + count + ".JPEG", src)
        count = count + 1
      })
    })
  }
